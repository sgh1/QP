...
More details about the Lennard-Jones potential and the velocity Verlet algorithm can be found in the class lectures(\cite{md1},\cite{md2}).  Also, the original paper on this topic can be found at \cite{original}.
...
The particle list class abstracts the group of particles (in our case, Argon atoms) used in the MD simulation. Abstracting the group of atoms rather than each atom often leads to simpler and less cumbersome code.  The methods contained in this class deal with the core of the MD simulation -- the Verlet algorithm and the thermostat -- and also manage fundamental observables.  Details will be discussed shortly. 
\vspace{5 mm}
...
The listing shown above should have enough comments to explain most of the methods, but some additional information will be discussed throughout the report.
...
\vspace{5 mm}
util.cpp contains the implementation for many of 'auxillary' functions required for the MD simulation.  It is noted that, for the Box-Muller function, the original, less robust version is used.  A more robust and efficient implementation is given in \citep{numerical}.
\vspace{5 mm}
